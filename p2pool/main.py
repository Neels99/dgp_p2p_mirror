...
def main(args, net, datadir_path, merged_urls, worker_endpoint):
    try:
        ...
        bitcoind = jsonrpc.HTTPProxy(url, dict(Authorization='Basic ' + base64.b64encode(
            args.bitcoind_rpc_username + ':' + args.bitcoind_rpc_password)), timeout=30)
        ...
        @defer.inlineCallbacks
        def poll_warnings():
            bitcoind_getnetworkinfo_var.set((yield deferral.retry('Error while calling getnetworkinfo:')(bitcoind.rpc_getnetworkinfo)()))
        yield poll_warnings()
        ...
        ss = p2pool_data.ShareStore(os.path.join(
            datadir_path, 'shares.'), net, share_cb, known_verified.add)
        ...
        global gnode
        gnode = node = p2pool_node.Node(
            factory, bitcoind, shares.values(), known_verified, net)
        yield node.start()
        ...
        if len(shares) > net.CHAIN_LENGTH:
            ...
            counts = p2pool_data.get_desired_version_counts(node.tracker, node.tracker.get_nth_parent_hash(
                previous_share.hash, net.CHAIN_LENGTH*9//10), net.CHAIN_LENGTH//10)
        ...
        print 'Listening for workers on %r port %i...' % (
        ...
        deferral.retry('Error binding to worker port:', traceback=False)(reactor.listenTCP)(
            worker_endpoint[1], serverfactory, interface=worker_endpoint[0])
                ...
                def signedOn(self):
                    ...
                    def new_share(share):
                        ...
                        if share.pow_hash <= share.header['bits'].target and abs(share.timestamp - time.time()) < 10*60:
                            ...
                            message = '\u001B[0m \x02%s BLOCK FOUND by %s! %s%064x \u001B[0m' % (net.NAME.upper(), bitcoin_data.script2_to_address(
        ...
        def status_thread():
            ...
            while True:
                ...
                try:
                    ...
                    my_shares_per_s = sum(datum['work']/dt/bitcoin_data.target_to_average_attempts(
                        datum['share_target']) for datum in datums)
                    this_str += '\n Local: %sH/s in last %s Local dead on arrival: %s Expected time to share: %s' % (
                        math.format(int(my_att_s)),
                        math.format_dt(dt),
                        math.format_binomial_conf(
                            sum(1 for datum in datums if datum['dead']), len(datums), 0.95),
                        math.format_dt(
                            1/my_shares_per_s) if my_shares_per_s else '???',
                    )
                    ...
                    if height > 2:
                        ...
                        stale_prop = p2pool_data.get_average_stale_prop(
                            node.tracker, node.best_share_var.value, min(60*60//net.SHARE_PERIOD, height))
                        real_att_s = p2pool_data.get_pool_attempts_per_second(
                            node.tracker, node.best_share_var.value, min(height - 1, 60*60//net.SHARE_PERIOD)) / (1 - stale_prop)
                        ...
                        this_str += '\n Shares: %i (%i orphan, %i dead) Stale rate: %s Efficiency: %s Current payout: %s %s' % (
                            ...
                            math.format_binomial_conf(
                                stale_orphan_shares + stale_doa_shares, shares, 0.95),
                            math.format_binomial_conf(
                                stale_orphan_shares + stale_doa_shares, shares, 0.95, lambda x: (1 - x)/(1 - stale_prop)),
                        ...
                        this_str += '\n Pool: %sH/s Stale rate: %.1f%% Expected time to block: %s' % (
                            ...
                            100*stale_prop,
                            math.format_dt(
                                2**256 / node.bitcoind_work.value['bits'].target / real_att_s),
                        )
